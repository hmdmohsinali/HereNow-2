...
export const addNewsRating = async (req, res) => {
    ...
    try {
      if (!newsId || !ratingValue) {
        return res
          .status(400)
          .json({ message: "NewsId and ratingValue are required." });
      }
      ...
      const rate = new NewsRatings({
        userId: userId, // same property used
        newsId: news._id,
        ratingValue,
      });
      ...
      return res
        .status(201)
        .json({ message: "Rating posted successfully on news", rating: savedRating });
    } catch (e) {
      console.log(e);
      return res
        .status(500)
        .json({ message: "An error occurred while posting the rating.", error: e.message });
    }
  };
